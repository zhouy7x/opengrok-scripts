...
PROJECT = 'v8'
lock_file = "/tmp/opengrok-repo-sync-%s.lock" % PROJECT
...
def check_update():
    cmd = "opengrok-mirror -U 'http://localhost:9090/' -I %s" % PROJECT
...
def run_update():
    ...
    opengrok-reindex-project -J=-d64 \
        -J=-XX:-UseGCOverheadLimit -J=-Xmx35g -J=-server -a /opengrok/lib/opengrok.jar \
        -t /opengrok/doc/logging.properties.template -p %s \
        -l "DEBUG" -d /log \
        -P %s -U 'http://localhost:9090/' -- \
        --renamedHistory on -r dirbased -G -m 256 \
        -c /usr/local/bin/ctags -U 'http://localhost:9090/' \
         -H %s
